...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    createNode: function (data) {
      ...
      var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + ' style= display:grid;' + (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data[opts.nextNode] ? ' data-next="' + data[opts.nextNode] + '"': ' data-next=""') + (data.parentId ? ' data-parent="' + data.parentId + '"' : '') + '>')
      ...
      $nodeDiv.append('<textarea class = "triggerWords" placeholder=" Триггеры" rows="1" aria-label="With textarea"></textarea>')
      $nodeDiv.append('<textarea class = "response" placeholder=" Ответ" rows="1" aria-label="With textarea"></textarea>')
      $nodeDiv.append('<label> <input class="endDialogue" type="checkbox" value="">Закончить диалог</label>')

      $nodeDiv.append('<button class="addNewNode" style="display:inline;">+</button>')
      if (data[opts.nodeId] != 0)
      {
        $nodeDiv.append('<button class="deleteThisNode">-</button>')
      }
